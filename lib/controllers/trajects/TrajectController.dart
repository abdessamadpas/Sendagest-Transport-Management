...
class TrajectsController extends GetxController {
  ...
  //  RxList<SelectedListItem> vehicles = <SelectedListItem>[].obs;
    ...
    fetchTrajects().then((_) {
      fetchClients().then((clients) {
        ...
        fetchVehicles().then((vehicles) {
          filterController.initializeVehicles(vehicles);
        });
      });
    });
  }
...
// fetch trajects with filters and save them in trajectsList
  Future<void> fetchTrajects() async {
    try {
      ...
      if (filterController.vehiculeTextEditingController.text.isNotEmpty) {
        data = data
            .where((item) =>
                item.Remorque ==
                filterController.vehiculeTextEditingController.text)
            .toList();
      }
...
//! for time filtration
  ...
  // fetch just Vehicles name from API
